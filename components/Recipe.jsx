...
const RecipeComponent = () => {
  ...
  const fetchRecipeData = async () => {
    try {
      const response = await fetch(
        `https://api.spoonacular.com/recipes/${selectedRecipeId}/information?apiKey=757784a98a374d4bbf1da35af525caaf&includeNutrition=true.`
      );
  ...
  const test = async () => {
    try {
      const response = await fetch(
        `https://api.spoonacular.com/recipes/complexSearch?query=${recipesearch}&number=2000&apiKey=757784a98a374d4bbf1da35af525caaf`
      );
      const data1 = await response.json();
      setreceipeiddata(data1);
      console.log(
        receipeiddata?.results[0].title,
        receipeiddata?.results[0].id
      );
    } catch (error) {
      console.error("Error fetching recipe data:", error);
    }
    ...
    <div className="h-auto overflow-x-hidden">
      ...
      <div style={anta.style} className="bg-darkblack h-auto">
        <div className="bg-cream h-40 flex items-center justify-between">
          <h1
            style={anta.style}
            className="p-10 text-gray-800 text-3xl font-semibold"
          >
            CRAVE YOUR SEARCHES HERE!
          </h1>
        ...
        <div className="p-10">
          <div className="bg-white rounded-xl">
            ...
            <h2 className="text-xl font-bold mt-4 p-4 bg-gray-200 text-gray-800">
              Ingredients:
            </h2>
